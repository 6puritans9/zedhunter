...
public class WallBuilder : MonoBehaviourPun
    {
        ...
        void BuildWall()
            {
                if (currentPreview != null && !IsOverlapping(currentPreview.transform.position))
                    {
                        GameObject newWall = PhotonNetwork.Instantiate(wallPrefab.name,
        ...
        public void UseRPC_AddBlock()
            {
                GameObject newWall = PhotonNetwork.Instantiate(wallPrefab.name, currentPreview.transform.position,
                    Quaternion.identity);
                newWall.layer = LayerMask.NameToLayer("Wall");
