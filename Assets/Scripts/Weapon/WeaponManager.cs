...
public class WeaponManager : MonoBehaviourPun, IPunPrefabPool
{
...
public class WeaponManager : MonoBehaviourPun
{
    ...
    void GetHitParticle(Vector3 position, Quaternion rotation)
    {
        ...
        if(particleSystem != null)
        {
            ...
            //if(photonView.IsMine)
            // 파티클 시스템 큐에 추가
            photonView.RPC("AddToHitParticlePool", RpcTarget.AllBuffered, particleSystem.gameObject.GetPhotonView().ViewID);
            if (photonView.IsMine)
                StartCoroutine(DeactivateParticleSystem(particleSystem));
        }
